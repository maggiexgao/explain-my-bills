  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div className={cn('p-2 rounded-lg', status ? statusColors[status] : 'bg-muted')}>
          {icon}
        </div>
  ...
  return (
    <div className="p-4 space-y-4 text-sm">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Badge variant="outline" className={cn(
          geoDebug.localityFound ? 'bg-success/10 text-success border-success/30' : 'bg-warning/10 text-warning border-warning/30'
        )}>
          {geoDebug.method === 'zip_exact' ? 'Exact Locality' :
           geoDebug.method === 'zip_to_state_avg' ? 'State Avg (from ZIP)' :
           geoDebug.method === 'state_avg' ? 'State Average' : 'National Default'}
        </Badge>
        <span className="text-muted-foreground">→</span>
        <Badge variant="outline">
          GPCI Applied: {geoDebug.localityFound ? 'Yes' : 'No'}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        ...
        <div className="p-2 rounded bg-muted/30">
          ...
          <div className={cn(
            'font-semibold',
            geoDebug.confidence === 'high' ? 'text-success' :
            geoDebug.confidence === 'medium' ? 'text-warning' : 'text-muted-foreground'
          )}>{geoDebug.confidence}</div>
        </div>
      </div>
    ...
    <div className="p-4 space-y-4 text-sm">
      ...
      <div className={cn(
        ...
        <div className="flex items-center gap-2">
          ...
          {reverseSearchReason && (
            <span className="text-xs text-muted-foreground">{reverseSearchReason}</span>
          )}
        </div>
        ...
        <div className="p-3 rounded-lg bg-muted/30 border border-border/30">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono">
              Sum: {formatCurrency(totalsReconciliation.sumLineCharges)}
            </span>
          </div>
        ...
        <div key={i} className="flex items-center justify-between py-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className={cn(
              ...
              c.confidence === 'medium' ? 'text-warning border-warning/30' : ''
            )}>{c.confidence}</Badge>
          </div>
        </div>
      ))}
    ...
    <div className="p-4 space-y-4 text-sm">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="p-3 rounded-lg bg-muted/30 text-center">
          ...
          <div className={cn(
            'text-lg font-bold',
            output.totals.multipleOfMedicare && output.totals.multipleOfMedicare > 3 ? 'text-destructive' :
            output.totals.multipleOfMedicare && output.totals.multipleOfMedicare > 2 ? 'text-warning' : ''
          )}>
            ...
            {pricedItems.slice(0, 10).map((item, i) => (
              <div key={i} className="p-2 text-xs grid grid-cols-4 gap-2">
                ...
                <div className="text-muted-foreground">
                  {item.feeSource === 'rvu_calc_local' ? 'GPCI adjusted' :
                   item.feeSource === 'rvu_calc_national' ? 'National RVU' :
                   item.feeSource === 'direct_fee' ? 'Direct fee' : '—'}
                </div>
      ...
      <div className="p-3 rounded-lg bg-muted/20 border border-border/30 text-xs">
        ...
        <code className="text-[10px] text-muted-foreground">
          Fee = [(Work RVU × Work GPCI) + (PE RVU × PE GPCI) + (MP RVU × MP GPCI)] × {output.metadata.benchmarkYearUsed === 2026 ? '34.6062' : 'CF'}
        </code>
        <div className="mt-2 text-muted-foreground">
          Data year: {output.metadata.benchmarkYearUsed} | 
          Locality: {output.metadata.localityName || 'National'} | 
      ...
      <div className="divide-y divide-border/30">
        ...
        <div>
          ...
          {expandedSections.has('location') && (
            <LocationResolutionSection geoDebug={data.geoDebug} />
          )}
        </div>
        ...
        <div>
          ...
          />
          {expandedSections.has('medicare') && (
            <MedicareCalculationSection output={data.benchmarkOutput} />
          )}
        </div>
      </div>
    </Card>
