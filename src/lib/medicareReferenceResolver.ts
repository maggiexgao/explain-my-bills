...
export async function resolveMedicareReferences(input: ResolverInput): Promise<ResolverOutput> {
  ...
  const gpci: GpciIndices | null = geoResolution.method !== 'national_default' 
    ? geoResolution.gpci 
    : null;
...
export function getSourceLabel(source: ReferenceSource): string {
  switch (source) {
    case 'mpfs_rvu_local': return 'MPFS (location-adjusted)';
    case 'mpfs_fee_national': return 'MPFS (national)';
    case 'opps_payment': return 'OPPS (hospital outpatient)';
    case 'dmepos_fee': return 'DMEPOS';
    case 'dmepen_fee': return 'DMEPEN';
...
export function getMatchStatusLabel(status: MatchStatus): string {
  switch (status) {
    ...
    case 'exists_not_priced': return 'Exists (not priced)';
    case 'missing_from_dataset': return 'Missing from datasets';
  }
}
